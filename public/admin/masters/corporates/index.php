...
async function loadData() {
    ...
    try {
        const response = await fetch(`../../../api/corporates.php?page=${currentPage}&limit=${limit}&_=${new Date().getTime()}`);
...
function renderTable(data, offset) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-muted">No corporates found.</td></tr>`;
        return;
    }
...
function renderPagination(p) {
    ...
    </ul></nav><div class="text-center text-muted small">Showing ${p.totalItems} total records</div>`;
